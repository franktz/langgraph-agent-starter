...
def test_sys_code_is_not_validated_by_default() -> None:
    ...
    ctx = service.resolve_request_context(
        req=ChatCompletionRequest(model="demo-hitl"),
        sys_code="unknown-system",
        session_id=None,
        user_id=None,
    )
    assert ctx.sys_code == "unknown-system"


def test_sys_code_validation_raises_when_enabled() -> None:
    config_provider = ConfigProvider(local_yaml_path="configs/local.yaml")
    config_provider.load_initial(
        None
    )
    config_provider._raw["api"]["auth"]["enabled"] = True  # type: ignore[attr-defined]
    config_provider._raw["api"]["auth"]["sys_codes"] = ["demo-system"]  # type: ignore[attr-defined]
    config_provider._conf = type(config_provider.conf)(config_provider._raw)  # type: ignore[attr-defined]
    logger_factory = setup_logging(config_provider)
    registry = WorkflowRegistry()
    catalog = WorkflowCatalogService(workflow_registry=registry)
    ...
    try:
        ...
        return
    raise AssertionError("expected invalid sys_code to raise")
